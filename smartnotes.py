...
from PyQt5.QtWidgets import QApplication,QWidget, QPushButton, QLabel, QListEdit, QTextEdit, QListWidget, QInputDialog, QHBoxLayout, QVBoxLayout,QFormLayout
import os
...
buttonnotes_create = QPushButton("Створити Нотатку")
buttonnotes_delete  = QPushButton("Видалити Нотатку")
buttonnotes__save  = QPushButton("Зберегти Нотатку")
...
def save_all_notes();
    for i, note in enumerate(notes):
        with open(f"{i}.txt", "w", encoding="utf-8"):
            file.write(note[0] + '\n')
            file.write(note[1] + '\n')
...
button_notes_create.clicked.connecy(add_note)
...
button_notes_save.clecked.connect(save_note)
